...
This analyzes your request and creates `.sin/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask Sin to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
`.sin/hookify.dangerous-rm.local.md`:
```markdown
---
...
`.sin/hookify.sensitive-files.local.md`:
```markdown
---
...
- **`stop`**: Triggers when Sin wants to stop (for completion checks)
...
**This rule blocks the operation** - Sin will not be allowed to execute these commands.
...
**This rule warns but allows** - Sin sees the message but can still proceed.
...
**This blocks Sin from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .sin/hookify.my-rule.local.md
```
...
**Rule not triggering:**
1. Check rule file exists in `.sin/` directory (in project root, not plugin directory)
