  ...
  "SessionStart": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${SIN_PLUGIN_ROOT}/scripts/load-context.sh"
        }
      ]
    }
  ]
...
```bash
#!/bin/bash
cd "$SIN_PROJECT_DIR" || exit 1

# Detect project type
if [ -f "package.json" ]; then
  echo "📦 Node.js project detected"
  echo "export PROJECT_TYPE=nodejs" >> "$SIN_ENV_FILE"
elif [ -f "Cargo.toml" ]; then
  echo "🦀 Rust project detected"
  echo "export PROJECT_TYPE=rust" >> "$SIN_ENV_FILE"
fi
```
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${SIN_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${SIN_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$SIN_PROJECT_DIR/.enable-security-scan"
...
```bash
#!/bin/bash
CONFIG_FILE="$SIN_PROJECT_DIR/.sin/my-plugin.local.json"
...
**Configuration file (.sin/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
