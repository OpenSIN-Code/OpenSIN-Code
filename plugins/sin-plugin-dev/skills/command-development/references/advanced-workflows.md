...
Deployment state saved to `.sin/deployment-state.local.md`:
...
```markdown
.sin/plugin-name-workflow.local.md:
...
Reading workflow state from .sin/plugin-name-workflow.local.md...

Current stage: @.sin/plugin-name-workflow.local.md
...
State file: @.sin/plugin-name-workflow.local.md
...
Creating: .sin/feature-complete.flag
...
if [ -f .sin/feature-complete.flag ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .sin/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .sin/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .sin/deployment-checkpoints.log
...
Written to .sin/deployment-state.local.md
...
Reading state: @.sin/deployment-state.local.md
...
rm .sin/deployment-state.local.md
