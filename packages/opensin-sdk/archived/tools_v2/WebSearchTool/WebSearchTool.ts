...
export const WebSearchTool = buildTool({
  ...
  async description(input) {
    return `OpenSIN wants to search the web for: ${input.query}`
  },
  userFacingName() {
    return 'Web Search'
  },
  ...
  isEnabled() {
    ...
    // Enable for Vertex AI with supported models (OpenSIN 4.0+)
    if (provider === 'vertex') {
      const supportsWebSearch =
        model.includes('opensin-opus-4') ||
        model.includes('opensin-sonnet-4') ||
        model.includes('opensin-haiku-4')
