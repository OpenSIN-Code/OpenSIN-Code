...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // opensin.ai connectors use a separate auth flow (handleOpenSINAIAuth in
      ...
      if (config.type === 'opensinai-proxy') {
        return {
          data: {
            ...
            message: `This is a opensin.ai MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
