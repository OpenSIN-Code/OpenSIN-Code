...
export async function resolveAttachments(
  ...
  // of the guard inside uploadBriefAttachment — OPENSIN.md: "helpers defined
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). OPENSIN_CODE_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes OPENSIN_CODE_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.OPENSIN_CODE_BRIEF_UPLOAD)
