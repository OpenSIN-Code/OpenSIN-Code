...
import { SEND_MESSAGE_TOOL_NAME } from '../../tools_v2/SendMessageTool/constants.js'
import { WEB_FETCH_TOOL_NAME } from '../../tools_v2/WebFetchTool/prompt.js'
import { WEB_SEARCH_TOOL_NAME } from '../../tools_v2/WebSearchTool/prompt.js'
import { isUsing3PServices } from '../../utils_v2/auth.js'
...
const OPENSIN_CODE_DOCS_MAP_URL =
  'https://code.opensin.com/docs/en/opensin_code_docs_map.md'
const CDP_DOCS_MAP_URL = 'https://platform.opensin.com/llms.txt'

export const OPENSIN_CODE_GUIDE_AGENT_TYPE = 'opensin-code-guide'

function getOpenSINCodeGuideBasePrompt(): string {
...
1. **OpenSIN Code** (the CLI tool): Installation, configuration, hooks, skills, MCP servers, keyboard shortcuts, IDE integrations, settings, and workflows.
...
- **OpenSIN Code docs** (${OPENSIN_CODE_DOCS_MAP_URL}): Fetch this for questions about the OpenSIN Code CLI tool, including:
...
- **OpenSIN Agent SDK docs** (${CDP_DOCS_MAP_URL}): Fetch this for questions about building agents with the SDK, including:
...
7. Reference local project files (OPENSIN.md, .opensin/ directory) when relevant using ${localSearchHint}
...
export const OPENSIN_CODE_GUIDE_AGENT: BuiltInAgentDefinition = {
  ...
  getSystemPrompt({ toolUseContext }) {
    ...
    // 2. Custom agents from .opensin/agents/
    const customAgents =
    ...
    const basePromptWithFeedback = `${getOpenSINCodeGuideBasePrompt()}
${feedbackGuideline}`
