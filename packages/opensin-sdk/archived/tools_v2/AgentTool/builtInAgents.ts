...
import { OPENSIN_CODE_GUIDE_AGENT } from './built-in/opensinCodeGuideAgent.js'
...
export function getBuiltInAgents(): AgentDefinition[] {
  ...
  if (
    isEnvTruthy(process.env.OPENSIN_AGENT_SDK_DISABLE_BUILTIN_AGENTS) &&
  ...
  if (feature('COORDINATOR_MODE')) {
    if (isEnvTruthy(process.env.OPENSIN_CODE_COORDINATOR_MODE)) {
  ...
  const isNonSdkEntrypoint =
    process.env.OPENSIN_CODE_ENTRYPOINT !== 'sdk-ts' &&
    process.env.OPENSIN_CODE_ENTRYPOINT !== 'sdk-py' &&
    process.env.OPENSIN_CODE_ENTRYPOINT !== 'sdk-cli'

  if (isNonSdkEntrypoint) {
    agents.push(OPENSIN_CODE_GUIDE_AGENT)
  }
