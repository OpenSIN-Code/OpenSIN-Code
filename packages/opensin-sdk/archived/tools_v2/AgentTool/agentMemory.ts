...
import { findCanonicalGitRoot } from '../../utils_v2/git.js'
import { sanitizePath } from '../../utils_v2/path.js'

// Persistent agent memory scope: 'user' (~/.opensin/agent-memory/), 'project' (.opensin/agent-memory/), or 'local' (.opensin/agent-memory-local/)
 ...
 * When OPENSIN_CODE_REMOTE_MEMORY_DIR is set, persists to the mount with project namespacing.
 * Otherwise, uses <cwd>/.opensin/agent-memory-local/<agentType>/.
 */
function getLocalAgentMemoryDir(dirName: string): string {
  if (process.env.OPENSIN_CODE_REMOTE_MEMORY_DIR) {
    return (
      join(
        process.env.OPENSIN_CODE_REMOTE_MEMORY_DIR,
        'projects',
        sanitizePath(
...
export function getAgentMemoryDir(
  ...
  switch (scope) {
    case 'project':
      return join(getCwd(), '.opensin', 'agent-memory', dirName) + sep
    case 'local':
...
export function isAgentMemoryPath(absolutePath: string): boolean {
  ...
  if (
    normalizedPath.startsWith(join(getCwd(), '.opensin', 'agent-memory') + sep)
  ) {
    return true
  }

  // Local scope: persisted to mount when OPENSIN_CODE_REMOTE_MEMORY_DIR is set, otherwise cwd-based
  if (process.env.OPENSIN_CODE_REMOTE_MEMORY_DIR) {
    if (
      normalizedPath.includes(sep + 'agent-memory-local' + sep) &&
      normalizedPath.startsWith(
        join(process.env.OPENSIN_CODE_REMOTE_MEMORY_DIR, 'projects') + sep,
      )
    ) {
      return true
    }
    ...
    normalizedPath.startsWith(
      join(getCwd(), '.opensin', 'agent-memory-local') + sep,
    )
  ) {
    return true
  }
...
export function getMemoryScopeDisplay(
  ...
  switch (memory) {
    ...
    case 'project':
      return 'Project (.opensin/agent-memory/)'
    case 'local':
 ...
 * @param scope 'user' for ~/.opensin/agent-memory/ or 'project' for .opensin/agent-memory/
 */
export function loadAgentMemoryPrompt(
  ...
  const coworkExtraGuidelines =
    process.env.OPENSIN_COWORK_MEMORY_EXTRA_GUIDELINES
