...
import { setScheduledTasksEnabled } from '../../bootstrap_system/state.js'
import type { ValidationResult } from '../../tools_v2/Tool.js'
import { buildTool, type ToolDef } from '../../tools_v2/Tool.js'
import { cronToHuman, parseCronExpression } from '../../utils_v2/cron.js'
import {
  addCronTask,
  getCronFilePath,
  listAllCronTasks,
...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .opensin/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this OpenSIN session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .opensin/scheduled_tasks.json'
      : 'Session-only (not written to disk, dies when OpenSIN exits)'
    return {
