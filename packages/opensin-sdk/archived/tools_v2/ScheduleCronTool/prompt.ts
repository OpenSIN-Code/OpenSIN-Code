...
import { DEFAULT_CRON_JITTER_CONFIG } from '../../utils_v2/cronTasks.js'
 ...
 * OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC are set; a `false` default would
 ...
 *
 * `OPENSIN_CODE_DISABLE_CRON` is a local override that wins over GB.
 */
export function isKairosCronEnabled(): boolean {
  return feature('AGENT_TRIGGERS')
    ? !isEnvTruthy(process.env.OPENSIN_CODE_DISABLE_CRON) &&
 ...
 * durable cron. Does NOT consult OPENSIN_CODE_DISABLE_CRON (that kills the whole
...
export function buildCronCreateDescription(durableEnabled: boolean): string {
  return durableEnabled
    ? 'Schedule a prompt to run at a future time — either recurring on a cron schedule, or once at a specific time. Pass durable: true to persist to .opensin/scheduled_tasks.json; otherwise session-only.'
    : 'Schedule a prompt to run at a future time within this OpenSIN session — either recurring on a cron schedule, or once at a specific time.'
}
...
By default (durable: false) the job lives only in this OpenSIN session — nothing is written to disk, and the job is gone when OpenSIN exits. Pass durable: true to write to .opensin/scheduled_tasks.json so the job survives restarts. Only use durable: true when the user explicitly asks for the task to persist ("keep doing this every day", "set this up permanently"). Most "remind me in 5 minutes" / "check back in an hour" requests should stay session-only.`
    : `## Session-only

Jobs live only in this OpenSIN session — nothing is written to disk, and the job is gone when OpenSIN exits.`

  const durableRuntimeNote = durableEnabled
    ? 'Durable jobs persist to .opensin/scheduled_tasks.json and survive session restarts — on next launch they resume automatically. One-shot durable tasks that were missed while the REPL was closed are surfaced for catch-up. Session-only jobs die with the process. '
    : ''
...
export function buildCronDeletePrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `Cancel a cron job previously scheduled with ${CRON_CREATE_TOOL_NAME}. Removes it from .opensin/scheduled_tasks.json (durable jobs) or the in-memory session store (session-only jobs).`
...
export function buildCronListPrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `List all cron jobs scheduled via ${CRON_CREATE_TOOL_NAME}, both durable (.opensin/scheduled_tasks.json) and session-only.`
