...
import { containsVulnerableUncPath } from '../../utils_v2/shell/readOnlyCommandValidation.js'
...
export async function powershellToolHasPermission(
  ...
  // `Write-Output $env:OPENSIN_API_KEY`, pushes allow to decisions[], and
  ...
  for (const { text: subCmd, element, statement } of subCommands) {
    ...
    if (
      ...
      // `Write-Output $env:OPENSIN_API_KEY`. Apply the same argLeaksValue
    ...
    // gate, `Set-Location ./.opensin; Set-Content ./settings.json '...'` would
    ...
    // .opensin/settings.json (a OpenSIN config file the path validator didn't check).
