...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .opensin is a dangerous directory
  // and internal editable paths live under ~/.opensin/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/opensin/) —
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (isPowerShellParameter(arg, argElementType)) {
      ...
      if (matchesParam(paramLower, config.pathParams)) {
        ...
        // SECURITY: A Variable elementType (e.g., $env:OPENSIN_API_KEY) in any
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.opensin; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.opensin/settings.json (OpenSIN's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.opensin`, validate subsequent statements with
  // cwd='./.opensin'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, OpenSIN Code may only access files in the allowed working directories for this session: ${dirListStr}.`
