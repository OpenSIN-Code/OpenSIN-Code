...
export const WebFetchTool = buildTool({
  ...
  async description(input) {
    ...
    try {
      const hostname = new URL(url).hostname
      return `OpenSIN wants to fetch content from ${hostname}`
    } catch {
      return `OpenSIN wants to fetch content from this URL`
    }
  },
  userFacingName() {
    return 'Fetch'
  ...
  async checkPermissions(input, context): Promise<PermissionDecision> {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `OpenSIN requested permissions to use ${WebFetchTool.name}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
  ...
  async prompt(_options) {
    ...
    // MCP tool count thresholds), invalidating the OpenSIN API prompt
...
- prompt: "${prompt}"`
    ...
    // mime-derived extension. Note it so OpenSIN can inspect the raw file
