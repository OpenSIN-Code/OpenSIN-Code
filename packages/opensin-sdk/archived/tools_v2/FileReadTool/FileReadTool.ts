import type { Base64ImageSource } from '@opensin-ai/sdk/resources/index.mjs'
 ...
 * Only matches files within the OpenSIN config directory (e.g., ~/.opensin).
...
function detectSessionFileType(
  ...
  // Only match files within the OpenSIN config directory
  ...
  // Session memory files: ~/.opensin/session-memory/*.md (including summary.md)
  if (
  ...
  // Session JSONL transcript files: ~/.opensin/projects/*/*.jsonl
  if (
...
const MITIGATION_EXEMPT_MODELS = new Set(['opensin-opus-4-6'])
