...
// OpenSIN configuration directory names
export const OPENSIN_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type OpenSINConfigDirectory = (typeof OPENSIN_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/opensins/opensin-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.opensin/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.opensin/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .opensin directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.opensin/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .opensin/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: OpenSINConfigDirectory,
  cwd: string,
): string[] {
  ...
  while (true) {
    ...
    const opensinSubdir = join(current, '.opensin', subdir)
    ...
    try {
      statSync(opensinSubdir)
      dirs.push(opensinSubdir)
    } catch (e: unknown) {
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getOpenSINConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.opensin', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .opensin/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .opensin/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .opensin/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (opensins/opensin-code#29599, #28182, #26992).
    //
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.opensin', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainOpenSINSubdir = join(canonicalRoot, '.opensin', subdir)
        if (!projectDirs.includes(mainOpenSINSubdir)) {
          projectDirs.push(mainOpenSINSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.opensin is
  ...
  (subdir: OpenSINConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * 3. Can be explicitly enabled via OPENSIN_CODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.opensin/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when OPENSIN_CODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.OPENSIN_CODE_USE_NATIVE_FILE_SEARCH)
