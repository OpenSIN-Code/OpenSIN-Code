...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // Respect OPENSIN_CODE_DISABLE_TERMINAL_TITLE — if the user opted out of
    // title changes, don't clear their existing title on exit either.
    if (!isEnvTruthy(process.env.OPENSIN_CODE_DISABLE_TERMINAL_TITLE)) {
...
function printResumeHint(): void {
  ...
  if (
    ...
    try {
      ...
      // Don't show resume hint if no session file exists (e.g., subcommands like `opensin update`)
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (OPENSIN_CODE_SESSIONEND_HOOKS_TIMEOUT_MS,
