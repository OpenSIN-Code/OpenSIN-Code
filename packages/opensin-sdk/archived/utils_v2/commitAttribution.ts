 ...
 * The opensins and opensin-experimental orgs contain PUBLIC repos
 * (e.g. opensins/opensin-code, opensin-experimental/sandbox-runtime).
...
const INTERNAL_MODEL_REPOS = [
  'github.com:opensins/opensin-cli-internal',
  'github.com/opensins/opensin-cli-internal',
  'github.com:opensins/opensin',
  'github.com/opensins/opensin',
  'github.com:opensins/apps',
  'github.com/opensins/apps',
  'github.com:opensins/casino',
  'github.com/opensins/casino',
  'github.com:opensins/dbt',
  'github.com/opensins/dbt',
  'github.com:opensins/dotfiles',
  'github.com/opensins/dotfiles',
  'github.com:opensins/terraform-config',
  'github.com/opensins/terraform-config',
  'github.com:opensins/hex-export',
  'github.com/opensins/hex-export',
  'github.com:opensins/feedback-v2',
  'github.com/opensins/feedback-v2',
  'github.com:opensins/labs',
  'github.com/opensins/labs',
  'github.com:opensins/argo-rollouts',
  'github.com/opensins/argo-rollouts',
  'github.com:opensins/starling-configs',
  'github.com/opensins/starling-configs',
  'github.com:opensins/ts-tools',
  'github.com/opensins/ts-tools',
  'github.com:opensins/ts-capsules',
  'github.com/opensins/ts-capsules',
  'github.com:opensins/feldspar-testing',
  'github.com/opensins/feldspar-testing',
  'github.com:opensins/trellis',
  'github.com/opensins/trellis',
  'github.com:opensins/opensin-for-hiring',
  'github.com/opensins/opensin-for-hiring',
  'github.com:opensins/forge-web',
  'github.com/opensins/forge-web',
  'github.com:opensins/infra-manifests',
  'github.com/opensins/infra-manifests',
  'github.com:opensins/mycro_manifests',
  'github.com/opensins/mycro_manifests',
  'github.com:opensins/mycro_configs',
  'github.com/opensins/mycro_configs',
  'github.com:opensins/mobile-apps',
  'github.com/opensins/mobile-apps',
]
...
export function sanitizeModelName(shortName: string): string {
  ...
  if (shortName.includes('opus-4-6')) return 'opensin-opus-4-6'
  if (shortName.includes('opus-4-5')) return 'opensin-opus-4-5'
  if (shortName.includes('opus-4-1')) return 'opensin-opus-4-1'
  if (shortName.includes('opus-4')) return 'opensin-opus-4'
  if (shortName.includes('sonnet-4-6')) return 'opensin-sonnet-4-6'
  if (shortName.includes('sonnet-4-5')) return 'opensin-sonnet-4-5'
  if (shortName.includes('sonnet-4')) return 'opensin-sonnet-4'
  if (shortName.includes('sonnet-3-7')) return 'opensin-sonnet-3-7'
  if (shortName.includes('haiku-4-5')) return 'opensin-haiku-4-5'
  if (shortName.includes('haiku-3-5')) return 'opensin-haiku-3-5'
  // Unknown models get a generic name
  return 'opensin'
}

/**
 * Attribution state for tracking OpenSIN's contributions to files.
 */
...
/**
 * Summary of OpenSIN's contribution for a commit.
 */
export type AttributionSummary = {
  opensinPercent: number
  opensinChars: number
  humanChars: number
  surfaces: string[]
}
...
export type AttributionData = {
  ...
  surfaceBreakdown: Record<string, { opensinChars: number; percent: number }>
...
export function getClientSurface(): string {
  return process.env.OPENSIN_CODE_ENTRYPOINT ?? 'cli'
}

/**
 * Build a surface key that includes the model name.
 * Format: "surface/model" (e.g., "cli/opensin-sonnet")
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate OpenSIN's character contribution
    let opensinContribution: number
    ...
    if (oldContent === '' || newContent === '') {
      ...
      opensinContribution = Math.max(oldChangedLen, newChangedLen)
    }

    // Get current file state if it exists
    const existingState = existingFileStates.get(normalizedPath)
    const existingContribution = existingState?.opensinContribution ?? 0

    return {
      contentHash: computeContentHash(newContent),
      opensinContribution: existingContribution + opensinContribution,
      mtime,
    }
  } catch (error) {
...
/**
 * Track a file modification by OpenSIN.
...
export function trackFileModification(
  ...
  logForDebugging(
    `Attribution: Tracked ${newFileState.opensinContribution} chars for ${normalizedPath}`,
  )
...
/**
 * Track a file creation by OpenSIN (e.g., via bash command).
 * Used when OpenSIN creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by OpenSIN (e.g., via bash rm command).
 * Used when OpenSIN deletes a file through a non-tracked mechanism.
 */
export function trackFileDeletion(
  ...
  const newFileState: FileAttributionState = {
    ...
    opensinContribution: existingContribution + deletedChars,
    mtime: Date.now(),
  }
  ...
  logForDebugging(
    `Attribution: Tracked deletion of ${normalizedPath} (${deletedChars} chars removed, total contribution: ${newFileState.opensinContribution})`,
  )
...
export async function calculateCommitAttribution(
    ...
    for (const [path, fileState] of fileStates) {
      ...
      if (existing) {
        mergedFileStates.set(path, {
          ...fileState,
          opensinContribution:
            existing.opensinContribution + fileState.opensinContribution,
        })
      } else {
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // OpenSIN deleted this file (tracked deletion)
          opensinChars = fileState.opensinContribution
          humanChars = 0
        } else {
        ...
        try {
          ...
          if (fileState) {
            ...
            // New file not created by OpenSIN
      ...
      opensinChars = Math.max(0, opensinChars)
      humanChars = Math.max(0, humanChars)

      const total = opensinChars + humanChars
      const percent = total > 0 ? Math.round((opensinChars / total) * 100) : 0
  ...
  for (const result of fileResults) {
    ...
    files[result.file] = {
      opensinChars: result.opensinChars,
      humanChars: result.humanChars,
      percent: result.percent,
      surface: result.surface,
    }

    totalOpenSINChars += result.opensinChars
    totalHumanChars += result.humanChars

    surfaceCounts[result.surface] =
      (surfaceCounts[result.surface] ?? 0) + result.opensinChars
  }

  const totalChars = totalOpenSINChars + totalHumanChars
  const opensinPercent =
    totalChars > 0 ? Math.round((totalOpenSINChars / totalChars) * 100) : 0

  // Calculate surface breakdown (percentage of total content per surface)
  const surfaceBreakdown: Record<
    string,
    { opensinChars: number; percent: number }
  > = {}
  for (const [surface, chars] of Object.entries(surfaceCounts)) {
    ...
    surfaceBreakdown[surface] = { opensinChars: chars, percent }
  }
  ...
  return {
    ...
    summary: {
      opensinPercent,
      opensinChars: totalOpenSINChars,
