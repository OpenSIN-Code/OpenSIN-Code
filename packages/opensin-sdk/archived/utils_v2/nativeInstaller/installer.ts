...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'opensin.exe' : 'opensin'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'opensin', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'opensin', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'opensin', 'locks'),
...
async function isPossibleOpenSINBinary(filePath: string): Promise<boolean> {
  try {
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nodeModulesDir = join(stagingPath, 'node_modules', '@opensin-ai')
    const entries = await readdir(nodeModulesDir)
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('opensin-cli-native-'),
    )
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/opensin to the version binary
  ...
  if (!(await isPossibleOpenSINBinary(executablePath))) {
...
async function versionIsAvailable(version: string): Promise<boolean> {
  ...
  return isPossibleOpenSINBinary(installPath)
}
...
async function updateLatest(
  ...
  if (
    ...
    (await isPossibleOpenSINBinary(executablePath))
  ) {
...
export async function checkInstall(
  ...
  // Check if opensin executable exists and is valid.
  ...
  // isPossibleOpenSINBinary stats the path internally, so we don't pre-check
  // with access() — that would be a TOCTOU between access and the stat.
  if (isWindows) {
    // On Windows it's a copied executable, not a symlink
    if (!(await isPossibleOpenSINBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but opensin command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleOpenSINBinary(absoluteTarget))) {
        messages.push({
          message: `OpenSIN symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but opensin command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleOpenSINBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid OpenSIN binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^opensin\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the opensin symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed opensin symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove opensin symlink: ${error}`))
  }
}

/**
 * Clean up old opensin aliases from shell configuration files
...
export async function cleanupShellAliases(): Promise<SetupMessage[]> {
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      const { filtered, hadAlias } = filterOpenSINAliases(lines)

      if (hadAlias) {
        await writeFileLines(configFile, filtered)
        messages.push({
          message: `Removed opensin alias from ${configFile}. Run: unalias opensin`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up opensin alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'opensin.cmd')
      const binPs1 = join(globalPrefix, 'opensin.ps1')
      const binExe = join(globalPrefix, 'opensin')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'opensin')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @opensin-ai/opensin-code
  const codePackageResult = await attemptNpmUninstall(
    '@opensin-ai/opensin-code',
  )
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@opensin-ai/opensin-code') {
  ...
  // Check for local installation at ~/.opensin/local
  const localInstallDir = join(homedir(), '.opensin', 'local')
