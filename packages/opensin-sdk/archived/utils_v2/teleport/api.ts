...
import { getOpenSINAIOAuthTokens } from '../auth.js'
...
export async function prepareApiRequest(): Promise<{
  ...
  if (accessToken === undefined) {
    throw new Error(
      'OpenSIN Code web sessions require authentication with a OpenSIN.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
 ...
 * @returns Headers object with Authorization, Content-Type, and opensin-version
 */
 ...
 * following the OpenSIN API messages spec.
 */
