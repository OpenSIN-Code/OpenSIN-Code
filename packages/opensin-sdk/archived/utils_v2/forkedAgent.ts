 ...
 * to share the parent's prompt cache. The OpenSIN API cache key is composed of:
 ...
 * opensin.ts (but only for older models that do not use adaptive thinking).
...
export type ForkedAgentParams = {
   ...
   * AND budget_tokens (via clamping in opensin.ts). If the fork uses cacheSafeParams
...
export async function runForkedAgent({
  ...
  // tool_uses are repaired downstream by ensureToolResultPairing in opensin.ts,
