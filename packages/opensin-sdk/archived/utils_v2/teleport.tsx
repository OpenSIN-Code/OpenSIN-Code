...
import { getTeleportErrors, TeleportError, type TeleportLocalErrorType } from '../../components_v2/TeleportError.js';
...
import { checkAndRefreshOAuthTokenIfNeeded, getOpenSINAIOAuthTokens } from './auth.js';
...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "opensin/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "opensin/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "opensin/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "opensin/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using OpenSIN Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'opensin/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getOpenSINAIOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('OpenSIN Code web sessions require authentication with a OpenSIN.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run opensin --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run opensin --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run opensin --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run opensin --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote OpenSIN.ai session with error handling and UI feedback.
...
export async function pollRemoteSessionEvents(sessionId: string, afterId: string | null = null, opts?: {
  ...
  const headers = {
    ...
    'opensin-beta': 'ccr-byoc-2025-07-29',
...
/**
 * Creates a remote OpenSIN.ai session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, OPENSIN_CODE_OAUTH_TOKEN is auto-injected from the
   ...
   * a new opensin/ branch. Sets allow_unrestricted_git_push on the source and
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        OPENSIN_CODE_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://opensin.ai/code' : '';
        let msg: string;
    ...
    // Select environment based on settings, then opensin_cloud preference, then first available.
    // Prefer opensin_cloud environments over byoc: opensin_cloud environments (e.g. "Default")
    ...
    let cloudEnv = environments.find(env => env.kind === 'opensin_cloud');
    ...
    if (options.useDefaultEnvironment && !cloudEnv) {
      logForDebugging(`No opensin_cloud in env list (${environments.length} envs); retrying fetchEnvironments`);
      const retried = await fetchEnvironments();
      cloudEnv = retried?.find(env => env.kind === 'opensin_cloud');
      if (!cloudEnv) {
        logError(new Error(`No opensin_cloud environment available after retry (got: ${(retried ?? environments).map(e => `${e.name} (${e.kind})`).join(', ')}). Silent byoc fallthrough would launch into a dead env — fail fast instead.`));
        return null;
      }
