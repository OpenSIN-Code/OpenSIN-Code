...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const opensinDir = join(home, '.opensin')

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(opensinDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(opensinDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(opensinDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: opensin completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const opensinBin = process.argv[1] || 'opensin'
  const result = await execFileNoThrow(opensinBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: opensin completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('opensin completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.opensin/.
 * Called after `opensin update` so completions stay in sync with the new binary.
 */
