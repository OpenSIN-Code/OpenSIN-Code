...
export async function getSessionEnvironmentScript(): Promise<string | null> {
  ...
  // Check for OPENSIN_ENV_FILE passed from parent process (e.g., HFI trajectory runner)
  // This allows venv/conda activation to persist across shell commands
  const envFile = process.env.OPENSIN_ENV_FILE
  if (envFile) {
    try {
      ...
      if (envScript) {
        ...
        logForDebugging(
          `Session environment loaded from OPENSIN_ENV_FILE: ${envFile} (${envScript.length} chars)`,
        )
      }
    } catch (e: unknown) {
      const code = getErrnoCode(e)
      if (code !== 'ENOENT') {
        logForDebugging(`Failed to read OPENSIN_ENV_FILE: ${errorMessage(e)}`)
      }
    }
  }
