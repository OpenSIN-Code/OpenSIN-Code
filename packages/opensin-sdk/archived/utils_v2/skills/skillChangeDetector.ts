...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directory (~/.opensin/skills)
  ...
  // User commands directory (~/.opensin/commands)
  ...
  // Project skills directory (.opensin/skills)
  ...
  // Project commands directory (.opensin/commands)
  ...
  for (const dir of getAdditionalDirectoriesForOpenSINMd()) {
    const additionalSkillsPath = platformPath.join(dir, '.opensin', 'skills')
    try {
