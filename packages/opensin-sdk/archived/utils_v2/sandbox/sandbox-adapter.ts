/**
 * Adapter layer that wraps @opensin-ai/sandbox-runtime with OpenSIN CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and OpenSIN CLI's
...
} from '@opensin-ai/sandbox-runtime'
import {
...
import { BASH_TOOL_NAME } from '../../tools_v2/BashTool/toolName.js'
import { FILE_EDIT_TOOL_NAME } from '../../tools_v2/FileEditTool/constants.js'
import { FILE_READ_TOOL_NAME } from '../../tools_v2/FileReadTool/prompt.js'
import { WEB_FETCH_TOOL_NAME } from '../../tools_v2/WebFetchTool/prompt.js'
import { errorMessage } from '../errors.js'
import { getOpenSINTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve OpenSIN Code-specific path patterns for sandbox-runtime.
 *
 * OpenSIN Code uses special path prefixes in permission rules:
...
/**
 * Convert OpenSIN Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and OpenSIN temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getOpenSINTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where OpenSIN Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.opensin', 'settings.json'))
    denyWrite.push(resolve(cwd, '.opensin', 'settings.local.json'))
  }

  // Block writes to .opensin/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .opensin/commands and
  // .opensin/agents but not .opensin/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full OpenSIN capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.opensin', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.opensin', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when OpenSIN's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See opensins/opensin-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * OpenSIN's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. opensins/opensin-code#29316.
 */
 ...
 * This is a OpenSIN CLI-specific function that updates local settings.
 */
...
/**
 * OpenSIN CLI sandbox manager - wraps sandbox-runtime with OpenSIN-specific features
 */
