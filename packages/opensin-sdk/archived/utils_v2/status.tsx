...
import { color, Text } from '../../ink_v2/index.js';
import type { MCPServerConnection } from '../services/mcp/types.js';
import { getAccountInformation, isOpenSINAISubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './opensinmd.js';
import { getDoctorDiagnostic } from './doctorDiagnostic.js';
import { getAWSRegion, getDefaultVertexRegion, isEnvTruthy } from './envUtils.js';
import { getDisplayPath } from './file.js';
import { formatNumber } from './format.js';
import { getIdeClientName, type IDEExtensionInstallationStatus, isJetBrainsIde, toIDEDisplayName } from './ide.js';
import { getOpenSINAiUserDefaultModelDescription, modelDisplayString } from './model/model.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    const opensinBaseUrl = process.env.OPENSIN_BASE_URL;
    if (opensinBaseUrl) {
      properties.push({
        label: 'OpenSIN base URL',
        value: opensinBaseUrl
      });
    }
    ...
    if (isEnvTruthy(process.env.OPENSIN_CODE_SKIP_BEDROCK_AUTH)) {
      properties.push({
    ...
    const gcpProject = process.env.OPENSIN_VERTEX_PROJECT_ID;
    if (gcpProject) {
      properties.push({
        label: 'GCP project',
        value: gcpProject
    ...
    const foundryBaseUrl = process.env.OPENSIN_FOUNDRY_BASE_URL;
    if (foundryBaseUrl) {
      properties.push({
    ...
    const foundryResource = process.env.OPENSIN_FOUNDRY_RESOURCE;
    if (foundryResource) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.OPENSIN_CODE_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.OPENSIN_CODE_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.OPENSIN_CODE_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.OPENSIN_CODE_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isOpenSINAISubscriber()) {
    const description = getOpenSINAiUserDefaultModelDescription();
