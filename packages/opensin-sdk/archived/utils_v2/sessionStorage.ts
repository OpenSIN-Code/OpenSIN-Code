...
// imports above. Named (not wildcard) per OPENSIN.md style; no collisions
...
import { builtInCommandNames } from '../../commands_v2/index.js'
...
export function getProjectsDir(): string {
  return join(getOpenSINConfigHomeDir(), 'projects')
}
...
function getEntrypoint(): string | undefined {
  return process.env.OPENSIN_CODE_ENTRYPOINT
}
 ...
 * This ensures tests with different OPENSIN_CONFIG_DIR values
...
class Project {
   ...
   * OPENSIN_CODE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.OPENSIN_CODE_SKIP_PROMPT_HISTORY)
    )
  }
 ...
 *
 * Streaming (opensin.ts:~2024) emits one AssistantMessage per content_block_stop
...
/**
 * Append a periodic task summary for `opensin ps`. Unlike ai-title this is
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.OPENSIN_CODE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // OPENSIN_CODE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.OPENSIN_CODE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.OPENSIN_CODE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
