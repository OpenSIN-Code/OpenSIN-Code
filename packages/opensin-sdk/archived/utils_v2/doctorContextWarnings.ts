...
import type { Tool, ToolPermissionContext } from '../../tools_v2/Tool.js'
...
async function checkOpenSINMdFiles(): Promise<ContextWarning | null> {
  ...
  const message =
    largeFiles.length === 1
      ? `Large OPENSIN.md file detected (${largeFiles[0]!.content.length.toLocaleString()} chars > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()})`
      : `${largeFiles.length} large OPENSIN.md files detected (each > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()} chars)`

  return {
    type: 'opensinmd_files',
    severity: 'warning',
    message,
    details,
...
export async function checkContextWarnings(
  ...
  const [opensinMdWarning, agentWarning, mcpWarning, unreachableRulesWarning] =
    await Promise.all([
      checkOpenSINMdFiles(),
