 ...
 * For ants with OPENSIN_CODE_TERMINAL_RECORDING=1: returns a path.
...
export function getRecordFilePath(): string | null {
  ...
  if (!isEnvTruthy(process.env.OPENSIN_CODE_TERMINAL_RECORDING)) {
    return null
  }
  // Record alongside the transcript.
  // Each launch gets its own file so --continue produces multiple recordings.
  const projectsDir = join(getOpenSINConfigHomeDir(), 'projects')
