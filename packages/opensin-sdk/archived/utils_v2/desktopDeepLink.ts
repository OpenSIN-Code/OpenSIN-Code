...
/**
 * Builds a deep link URL for OpenSIN Desktop to resume a CLI session.
 * Format: opensin://resume?session={sessionId}&cwd={cwd}
 * In dev mode: opensin-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'opensin-dev' : 'opensin'
...
/**
 * Check if OpenSIN Desktop app is installed.
 * On macOS, checks for /Applications/OpenSIN.app.
 * On Linux, checks if xdg-open can handle opensin:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for OpenSIN.app in /Applications
    return pathExists('/Applications/OpenSIN.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for opensin://
...
/**
 * Detect the installed OpenSIN Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/OpenSIN.app/Contents/Info.plist',
...
/**
 * Build and open a deep link to resume the current session in OpenSIN Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'OpenSIN Desktop is not installed. Install it from https://opensin.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open OpenSIN Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
