...
import { getSdkBetas, getSessionId } from '../../bootstrap_system/state.js'
...
} from '../../tools_v2/AgentTool/constants.js'
import { getOpenSINApiKeyWithSource } from '../auth.js'
...
export function buildSystemInitMessage(inputs: SystemInitInputs): SDKMessage {
  ...
  const initMessage: SDKMessage = {
    ...
    slash_commands: inputs.commands
      ...
      .map(c => c.name),
    apiKeySource: getOpenSINApiKeyWithSource().source as ApiKeySource,
    betas: getSdkBetas(),
    opensin_code_version: MACRO.VERSION,
