 ...
 * rule (src/OPENSIN.md): the dispatcher closure must persist across calls so
...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another OpenSIN session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'OpenSIN is using your computer · press Esc to stop' : 'OpenSIN is using your computer · press Ctrl+C to stop',
...
export function getComputerUseMCPToolOverrides(toolName: string): ComputerUseMCPToolOverrides {
  const call: CallOverride = async (args, context: ToolUseContext) => {
    ...
    // MCP content blocks → OpenSIN API blocks. CU only produces text and
