...
import { clearMemoryFileCaches } from './opensinmd.js'
import {
 ...
 * Used for computing initial state before render (per OPENSIN.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `opensin -r {newSessionId}`
  ...
  // Compute initial state before render (per OPENSIN.md guidelines)
