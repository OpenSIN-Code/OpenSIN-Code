...
/**
 * All tool names from BROWSER_TOOLS in @ant/opensin-for-chrome-mcp.
...
function renderChromeToolUseMessage(input: Record<string, unknown>, toolName: ChromeToolName, verbose: boolean): React.ReactNode {
  ...
  if (typeof tabId === 'number') {
    trackOpenSINInChromeTabId(tabId);
  }
...
/**
 * Renders a clickable "View Tab" link for OpenSIN in Chrome MCP tools.
 * Returns null if:
 * - The tool is not a OpenSIN in Chrome MCP tool
...
/**
 * Custom tool result message rendering for opensin-in-chrome tools.
...
/**
 * Returns tool method overrides for OpenSIN in Chrome MCP tools. Use this to customize
 * rendering for chrome tools in a single spread operation.
 */
export function getOpenSINInChromeMCPToolOverrides(toolName: string): {
  ...
  return {
    userFacingName(_input?: Record<string, unknown>) {
      ...
      return `OpenSIN in Chrome[${displayName}]`;
    },
