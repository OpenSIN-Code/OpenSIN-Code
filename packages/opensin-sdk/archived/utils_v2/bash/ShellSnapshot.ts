...
/**
 * Generates OpenSIN Code specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getOpenSINCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in OpenSIN's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and OpenSIN Code content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const opensinCodeContent = await getOpenSINCodeSnapshotContent()
 ...
 *
 * This function is a critical part of OpenSIN CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows OpenSIN CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with OpenSIN Code defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getOpenSINConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.OPENSIN_CODE_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - OpenSIN home: ${getOpenSINConfigHomeDir()}`)
