...
/**
 * Convert a PersistedStatsCache to OpenSINCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all OpenSIN Code sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateOpenSINCodeStats(): Promise<OpenSINCodeStats> {
 ...
 */
export async function aggregateOpenSINCodeStatsForRange(
  range: StatsDateRange,
): Promise<OpenSINCodeStats> {
  if (range === 'all') {
    return aggregateOpenSINCodeStats()
  }
  ...
  return processedStatsToOpenSINCodeStats(stats)
}

/**
 * Convert ProcessedStats to OpenSINCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToOpenSINCodeStats(
...
function getEmptyStats(): OpenSINCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
