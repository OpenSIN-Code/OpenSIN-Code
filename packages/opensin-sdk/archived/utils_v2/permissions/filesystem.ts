...
/**
 * If filePath is inside a .opensin/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .opensin/ (settings.json, hooks/, etc.).
 */
export function getOpenSINSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.opensin', 'skills')),
      prefix: '/.opensin/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.opensin', 'skills')),
      prefix: '~/.opensin/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.opensin/skills/*/**' which matches ALL skills. Return null
...
export function isOpenSINSettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.opensin/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.opensin${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.opensin${sep}settings.local.json`)
  ) {
    // Include .opensin/settings.json even for other projects
    return true
  }
...
// Always ask when OpenSIN Code tries to edit its own config files
function isOpenSINConfigFilePath(filePath: string): boolean {
  if (isOpenSINSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .opensin/commands or .opensin/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.opensin', 'commands')
  const agentsDir = join(getOriginalCwd(), '.opensin', 'agents')
  const skillsDir = join(getOriginalCwd(), '.opensin', 'skills')
 ...
 * Path format: ~/.opensin/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for OpenSIN to write temporary files.
...
/**
 * Returns the user-specific OpenSIN temp directory name.
 * On Unix: 'opensin-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'opensin' (tmpdir() is already per-user)
 */
export function getOpenSINTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'opensin'
  }
...
/**
 * Returns the OpenSIN temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/opensin-{uid}/ (resolved to /private/tmp/opensin-{uid}/ on macOS)
 * - On Windows: {tmpdir}/opensin/ (e.g., C:\Users\{user}\AppData\Local\Temp\opensin\)
 * This is a per-user temporary directory used by OpenSIN Code for all temp files.
 *
 ...
 * resolution, paths like /tmp/opensin-{uid}/... wouldn't match /private/tmp/opensin-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (OPENSIN_CODE_TMPDIR env + platform) are
// fixed at startup, and the realpath of the system tmp dir does not change mid-session.
export const getOpenSINTempDir = memoize(function getOpenSINTempDir(): string {
  const baseTmpDir =
    process.env.OPENSIN_CODE_TMPDIR ||
  ...
  return join(resolvedBaseTmpDir, getOpenSINTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getOpenSINTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/opensin-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getOpenSINTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/opensin-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/opensin-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .opensin/worktrees/ is a structural path (where OpenSIN stores
      // git worktrees), not a user-created dangerous directory. Skip the .opensin
      // segment when it's followed by 'worktrees'. Any nested .opensin directories
      ...
      if (dir === '.opensin') {
        ...
        if (
          ...
          break // Skip this .opensin, continue checking other segments
        }
      }
 ...
 * - 8.3 short names (e.g., GIT~1, OPENSIN~1, SETTIN~1.JSON)
 * - Long path prefixes (e.g., \\?\C:\..., \\.\C:\..., //?/C:/..., //./C:/...)
 * - Trailing dots and spaces (e.g., .git., .opensin , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: GIT~1, OPENSIN~1, SETTIN~1.JSON, BASHRC~1
  if (/~\d/.test(path)) {
    return true
  }
  ...
  // Examples: .git., .opensin , .bashrc..., settings.json.
 ...
 * - OpenSIN config files (.opensin/settings.json, .opensin/commands/, .opensin/agents/)
 * - MCP CLI state files (managed internally by OpenSIN Code)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `OpenSIN requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for OpenSIN config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isOpenSINConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `OpenSIN requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `OpenSIN requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .opensin/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `OpenSIN requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `OpenSIN requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `OpenSIN requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `OpenSIN requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .opensin is a dangerous directory
  ...
  // 1.6. Check for .opensin/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .opensin/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .opensin/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.opensin) rule in userSettings (e.g. from sandbox
  ...
  // "allow OpenSIN to edit its own settings for this session" option actually works.
  const opensinFolderAllowRule = matchingRuleForInput(
    path,
    {
      ...toolPermissionContext,
      alwaysAllowRules: {
  ...
  if (opensinFolderAllowRule) {
    // Check if this rule is scoped under .opensin/ (project or global).
    // Accepts both the broad patterns ('/.opensin/**', '~/.opensin/**') and
    // narrowed ones like '/.opensin/skills/my-skill/**' so users can grant
    ...
    // '/.opensin/../**' from leaking this bypass outside .opensin/.
    const ruleContent = opensinFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      (ruleContent.startsWith(OPENSIN_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
        ruleContent.startsWith(
          GLOBAL_OPENSIN_FOLDER_PERMISSION_PATTERN.slice(0, -2),
        )) &&
      ...
      return {
        ...
        decisionReason: {
          type: 'rule',
          rule: opensinFolderAllowRule,
        },
      }
    }
  }

  // 1.7. Check comprehensive safety validations (Windows patterns, OpenSIN config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .opensin/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.opensin/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getOpenSINSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.opensin/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    const jobDir = process.env.OPENSIN_JOB_DIR
    if (jobDir) {
      const jobsRoot = join(getOpenSINConfigHomeDir(), 'jobs')
      ...
      // the case where ~/.opensin is a symlink (e.g. to /data/opensin-config).
  ...
  // ~/.opensin/, which is in DANGEROUS_DIRECTORIES. The OPENSIN_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .opensin/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs OpenSIN to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .opensin/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .opensin/ only (not ~/.opensin/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.opensin', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/opensin/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.opensin/tasks/) for swarm task coordination
  const tasksDir = join(getOpenSINConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.opensin/teams/) for swarm coordination
  const teamsReadDir = join(getOpenSINConfigHomeDir(), 'teams') + sep
  if (
