...
import { getSessionCreatedTeams } from '../../bootstrap_system/state.js'
...
export async function cleanupTeamDirectories(teamName: string): Promise<void> {
  ...
  // Clean up team directory (~/.opensin/teams/{team-name}/)
  const teamDir = getTeamDir(teamName)
  try {
  ...
  // Clean up tasks directory (~/.opensin/tasks/{taskListId}/)
