// Scheduled prompts, stored in <project>/.opensin/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.opensin', 'scheduled_tasks.json')
...
/**
 * Read and parse .opensin/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .opensin/scheduled_tasks.json with the given tasks. Creates .opensin/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.opensin'), { recursive: true })
 ...
 * written to .opensin/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while OpenSIN
