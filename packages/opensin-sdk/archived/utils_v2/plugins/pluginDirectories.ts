 ...
 * - CLI flag: --cowork
 * - Environment variable: OPENSIN_CODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via OPENSIN_CODE_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable OPENSIN_CODE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.OPENSIN_CODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. OPENSIN_CODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.opensin/plugins or ~/.opensin/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when OPENSIN_CODE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.opensin/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.OPENSIN_CODE_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getOpenSINConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point OPENSIN_CODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.OPENSIN_CODE_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${OPENSIN_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${OPENSIN_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${OPENSIN_PLUGIN_DATA} is present
