 ...
 *
 * This module is responsible for discovering, loading, and validating OpenSIN Code plugins
 ...
 * Format: ~/.opensin/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.opensin/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.OPENSIN_CODE_REMOTE)
 ...
 * OPENSIN_CODE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.opensin-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".opensin-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".opensin-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.opensin-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".opensin-plugin/name")
 ...
 *
 * OPENSIN_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getOpenSINCodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .opensin-plugin/ directory (for export)
