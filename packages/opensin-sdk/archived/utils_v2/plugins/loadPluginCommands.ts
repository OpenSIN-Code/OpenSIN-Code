...
import { getInlinePlugins, getSessionId } from '../../bootstrap_system/state.js'
...
function createPluginCommand(
  ...
  try {
    ...
    // Substitute ${OPENSIN_PLUGIN_ROOT} in allowed-tools before parsing
    ...
    return {
      ...
      async getPromptForCommand(args, context) {
        ...
        // Replace ${OPENSIN_PLUGIN_ROOT} and ${OPENSIN_PLUGIN_DATA} with their paths
        ...
        // Replace ${OPENSIN_SKILL_DIR} with this specific skill's directory.
        // Distinct from ${OPENSIN_PLUGIN_ROOT}: a plugin can contain multiple
        // skills, so OPENSIN_PLUGIN_ROOT points to the plugin root while
        // OPENSIN_SKILL_DIR points to the individual skill's subdirectory.
        ...
        // Replace ${OPENSIN_SESSION_ID} with the current session ID
        finalContent = finalContent.replace(
          /\$\{OPENSIN_SESSION_ID\}/g,
          getSessionId(),
        )
