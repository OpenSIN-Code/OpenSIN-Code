// Scheduler lease lock for .opensin/scheduled_tasks.json.
//
// When multiple OpenSIN sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.opensin', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .opensin/ doesn't exist yet — create it and retry once. In steady
