...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.opensin/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.opensin/settings.json)'
    case 'projectSettings':
      return 'Project settings (.opensin/settings.json)'
    case 'localSettings':
      return 'Local settings (.opensin/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.opensin/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.opensin/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by OpenSIN Code)'
    default:
