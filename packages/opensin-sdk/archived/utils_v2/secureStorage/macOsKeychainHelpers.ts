...
export function getMacOsKeychainStorageServiceName(
  ...
  const isDefaultDir = !process.env.OPENSIN_CONFIG_DIR
  ...
  return `OpenSIN Code${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}
...
//
// The sync read() path takes ~500ms per `security` spawn. With 50+ opensin.ai
