...
function getSessionsDir(): string {
  return join(getOpenSINConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`opensin --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.OPENSIN_CODE_SESSION_KIND
...
/**
 * True when this REPL is running inside a `opensin --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `opensin ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.OPENSIN_CODE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.OPENSIN_CODE_SESSION_NAME,
              logPath: process.env.OPENSIN_CODE_SESSION_LOG,
              agent: process.env.OPENSIN_CODE_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `opensin ps` sparkline
...
/**
 * Push live activity state for `opensin ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See opensins/opensin-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.opensin/sessions/ is shared with Windows-native OpenSIN (symlink
      // or OPENSIN_CONFIG_DIR), a Windows PID won't be probeable from WSL
