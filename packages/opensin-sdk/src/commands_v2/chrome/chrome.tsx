...
import { type OptionWithDescription, Select } from '../../components_v2/CustomSelect/select.js';
...
import { OPENSIN_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils_v2/opensinInChrome/common.js';
...
type Props = {
  ...
  isOpenSINAISubscriber: boolean;
  isWSL: boolean;
};
function OpenSINInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              opensinInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isOpenSINAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>OpenSIN in Chrome works with the Chrome extension to let you control your browser directly from OpenSIN Code. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">OpenSIN in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t8;
  if ($[23] !== isOpenSINAISubscriber) {
    t8 = true && !isOpenSINAISubscriber && <Text color="error">OpenSIN in Chrome requires a opensin.ai subscription.</Text>;
    $[23] = isOpenSINAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>opensin --chrome</Text><Text dimColor={true}> or </Text><Text>opensin --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites OpenSIN can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text dimColor={true}>Learn more: https://code.opensin.com/docs/en/chrome</Text>;
    $[33] = t10;
  } else {
    t10 = $[33];
  }
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="OpenSIN in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
function _temp2(c) {
  return c.name === OPENSIN_IN_CHROME_MCP_SERVER_NAME;
}
function _temp(s) {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <OpenSINInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.opensinInChromeDefaultEnabled} isOpenSINAISubscriber={isSubscriber} isWSL={isWSL} />;
};
