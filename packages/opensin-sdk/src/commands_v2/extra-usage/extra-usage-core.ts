...
import { hasOpenSINAiBillingAccess } from '../../utils/billing'
...
export async function runExtraUsage(): Promise<ExtraUsageResult> {
  ...
  const hasBillingAccess = hasOpenSINAiBillingAccess()

  if (!hasBillingAccess && isTeamOrEnterprise) {
    // Mirror apps/opensin-ai useHasUnlimitedOverage(): if overage is enabled
  ...
  const url = isTeamOrEnterprise
    ? 'https://opensin.ai/admin-settings/usage'
    : 'https://opensin.ai/settings/usage'
