...
import { validateManifest } from '../../utils_v2/plugins/validatePlugin.js';
  ...
  if ($[0] !== onComplete || $[1] !== path) {
    t1 = () => {
      const runValidation = async function runValidation() {
        if (!path) {
          onComplete("Usage: /plugin validate <path>\n\nValidate a plugin or marketplace manifest file or directory.\n\nExamples:\n  /plugin validate .opensin-plugin/plugin.json\n  /plugin validate /path/to/plugin-directory\n  /plugin validate .\n\nWhen given a directory, automatically validates .opensin-plugin/marketplace.json\nor .opensin-plugin/plugin.json (prefers marketplace if both exist).\n\nOr from the command line:\n  opensin plugin validate <path>");
          return;
        }
        ;
        try {
