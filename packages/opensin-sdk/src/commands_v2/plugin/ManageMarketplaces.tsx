...
import { shouldSkipPluginAutoupdate } from '../../utils_v2/config.js';
import { errorMessage } from '../../utils_v2/errors.js';
import { clearAllCaches } from '../../utils_v2/plugins/cacheUtils.js';
import { createPluginId, formatMarketplaceLoadingErrors, getMarketplaceSourceDisplay, loadMarketplacesWithGracefulDegradation } from '../../utils_v2/plugins/marketplaceHelpers.js';
import { loadKnownMarketplacesConfig, refreshMarketplace, removeMarketplaceSource, setMarketplaceAutoUpdate } from '../../utils_v2/plugins/marketplaceManager.js';
import { updatePluginsForMarketplaces } from '../../utils_v2/plugins/pluginAutoupdate.js';
import { loadAllPlugins } from '../../utils_v2/plugins/pluginLoader.js';
import { isMarketplaceAutoUpdate } from '../../utils_v2/plugins/schemas.js';
...
export function ManageMarketplaces({
  ...
  useEffect(() => {
    async function loadMarketplaces() {
      try {
        ...
        // Sort: opensin-plugin-directory first, then alphabetically
        states.sort((a, b) => {
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                Auto-update enabled. OpenSIN Code will automatically update this
  ...
  return <Box flexDirection="column">
              ...
              <Box flexDirection="column" flexGrow={1}>
                <Box flexDirection="row" gap={1}>
                  <Text bold strikethrough={state.pendingRemove} dimColor={state.pendingRemove}>
                    {state.name === 'opensin-plugins-official' && <Text color="opensin">✻ </Text>}
                    {state.name}
                    {state.name === 'opensin-plugins-official' && <Text color="opensin"> ✻</Text>}
                  </Text>
      ...
      {isProcessing && <Box marginTop={1}>
          <Text color="opensin">Processing changes…</Text>
        </Box>}
