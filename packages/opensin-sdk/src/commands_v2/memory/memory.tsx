...
function MemoryCommand({
  ...
  const handleSelectMemoryFile = async (memoryPath: string) => {
    try {
      // Create opensin directory if it doesn't exist (idempotent with recursive)
      if (memoryPath.includes(getOpenSINConfigHomeDir())) {
        await mkdir(getOpenSINConfigHomeDir(), {
          recursive: true
        });
      }
  ...
  return <Dialog title="Memory" onCancel={handleCancel} color="remember">
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <Text dimColor>
            Learn more: <Link url="https://code.opensin.com/docs/en/memory" />
          </Text>
        </Box>
      </Box>
    </Dialog>;
