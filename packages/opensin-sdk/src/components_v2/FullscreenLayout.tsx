...
import { openBrowser, openPath } from '../../utils_v2/browser.js';
...
export function useUnseenDivider(messageCount: number): {
  ...
  const onScrollAway = useCallback((handle: ScrollBoxHandle) => {
    ...
    //   at max (Sarah Deaton, #opensin-code-feedback 2026-03-15)
 ...
 * users think of as "a new message from OpenSIN" — not raw assistant entries
 ...
 * until OpenSIN's text response lands.
 */
 ...
 *
 * Fullscreen mode defaults on for ants (OPENSIN_CODE_NO_FLICKER=0 to opt out)
 * and off for external users (OPENSIN_CODE_NO_FLICKER=1 to opt in).
...
// conversation turn's prompt above the viewport so you know what OpenSIN was
