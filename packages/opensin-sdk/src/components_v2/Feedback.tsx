...
import { getLastAPIRequest } from '../../bootstrap_system/state.js';
import { logEventTo1P } from '../services/analytics/firstPartyEventLogger.js';
import { type AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS, logEvent } from '../services/analytics/index.js';
import { getLastAssistantMessage, normalizeMessagesForAPI } from '../../utils_v2/messages.js';
...
import { type GitRepoState, getGitState, getIsGit } from '../../utils_v2/git.js';
import { getAuthHeaders, getUserAgent } from '../../utils_v2/http.js';
import { getInMemoryErrors, logError } from '../../utils_v2/log.js';
import { isEssentialTrafficOnly } from '../../utils_v2/privacyLevel.js';
import { extractTeammateTranscriptsFromTasks, getTranscriptPath, loadAllSubagentTranscriptsFromDisk, MAX_TRANSCRIPT_READ_BYTES } from '../../utils_v2/sessionStorage.js';
...
const GITHUB_ISSUES_REPO_URL = "external" === 'ant' ? 'https://github.com/opensins/opensin-cli-internal/issues' : 'https://github.com/opensins/opensin-code/issues';
type Props = {
...
export function redactSensitiveInfo(text: string): string {
  ...
  // Opensin API keys (sk-ant...) with or without quotes
      ...
      {step === 'consent' && <Box flexDirection="column">
          ...
          <Box marginTop={1}>
            <Text wrap="wrap" dimColor>
              ...
              OpenSIN Code&apos;s functionality (eg. to reduce the risk of bugs
...
async function generateTitle(description: string, abortSignal: AbortSignal): Promise<string> {
  try {
    const response = await queryHaiku({
      systemPrompt: asSystemPrompt(['Generate a concise, technical issue title (max 80 chars) for a public GitHub issue based on this bug report for OpenSIN Code.', 'OpenSIN Code is an agentic coding CLI based on the Opensin API.', 'The title should:', '- Include the type of issue [Bug] or [Feature Request] as the first thing in the title', '- Be concise, specific and descriptive of the actual problem', '- Use technical terminology appropriate for a software issue', '- For error messages, extract the key error (e.g., "Missing Tool Result Block" rather than the full message)', '- Be direct and clear for developers to understand the problem', '- If you cannot determine a clear issue, use "Bug Report: [brief description]"', '- Any LLM API errors are from the Opensin API, not from any other model provider', 'Your response will be directly used as the title of the Github issue, and as such should not contain any other commentary or explaination', 'Examples of good titles include: "[Bug] Auto-Compact triggers to soon", "[Bug] Opensin API Error: Missing Tool Result Block", "[Bug] Error: Invalid Model Name for Opus"']),
...
async function submitFeedback(data: FeedbackData, signal?: AbortSignal): Promise<{
  ...
  try {
    ...
    const response = await axios.post('https://api.opensin.com/api/opensin_cli_feedback', {
