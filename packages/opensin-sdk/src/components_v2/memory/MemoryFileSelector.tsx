...
import { openPath } from '../../utils_v2/browser.js';
import { getMemoryFiles, type MemoryFileInfo } from '../../utils_v2/opensinmd.js';
...
import { projectIsInGitRepo } from '../../utils_v2/memory/versions.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getOpenSINConfigHomeDir(), "OPENSIN.md");
  const projectMemoryPath = join(getOriginalCwd(), "OPENSIN.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.opensin/OPENSIN.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./OPENSIN.md`;
      } else {
        if (file.parent) {
