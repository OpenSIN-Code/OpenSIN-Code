...
import { useDynamicConfig } from '../../hooks_v2/useDynamicConfig.js';
...
export function useFeedbackSurvey(messages: Message[], isLoading: boolean, submitCount: number, surveyType: FeedbackSurveyType = 'session', hasActivePrompt: boolean = false): {
  ...
  const shouldOpen = useMemo(() => {
    ...
    if (process.env.OPENSIN_FORCE_DISPLAY_SURVEY && !feedbackSurvey.timeLastShown) {
      return true;
    }
    if (!isModelAllowed) {
      return false;
    }
    if (isEnvTruthy(process.env.OPENSIN_CODE_DISABLE_FEEDBACK_SURVEY)) {
      return false;
    }
