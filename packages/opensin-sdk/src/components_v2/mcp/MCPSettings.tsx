    ...
    t5 = () => {
      ...
      const prepareServers = async function prepareServers() {
        const serverInfos = await Promise.all(filteredClients.map(async client_0 => {
          ...
          const isOpenSINAIProxy = client_0.config.type === "opensinai-proxy";
          let isAuthenticated = undefined;
          if (isSSE || isHTTP) {
            const authProvider = new OpenSINAuthProvider(client_0.name, client_0.config as McpSSEServerConfig | McpHTTPServerConfig);
          ...
          if (isOpenSINAIProxy) {
            return {
              ...baseInfo,
              transport: "opensinai-proxy" as const,
              isAuthenticated: false,
              config: client_0.config as McpOpenSINAIProxyServerConfig
            };
          } else {
            if (isSSE) {
              return {
  ...
  if ($[10] !== agentMcpServers.length || $[11] !== filteredClients.length || $[12] !== onComplete || $[13] !== servers.length) {
    t7 = () => {
      ...
      if (servers.length === 0 && agentMcpServers.length === 0) {
        onComplete("No MCP servers configured. Please run /doctor if this is unexpected. Otherwise, run `opensin mcp --help` or visit https://code.opensin.com/docs/en/mcp to learn more.");
      }
    };
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "opensinai-proxy" ? "opensin.ai" : "OpenSIN Code";
