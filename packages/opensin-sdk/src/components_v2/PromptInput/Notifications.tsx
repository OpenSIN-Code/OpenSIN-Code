...
import { useIdeConnectionStatus } from '../../hooks_v2/useIdeConnectionStatus.js';
import type { IDESelection } from '../../hooks_v2/useIdeSelection.js';
import { useMainLoopModel } from '../../hooks_v2/useMainLoopModel.js';
import { useVoiceEnabled } from '../../hooks_v2/useVoiceEnabled.js';
...
import { getApiKeyHelperElapsedMs, getConfiguredApiKeyHelper, getSubscriptionType } from '../../utils_v2/auth.js';
...
import { setEnvHookNotifier } from '../../utils_v2/hooks/fileChangedWatcher.js';
...
export function Notifications(t0) {
  ...
  const opensinAiLimits = useOpenSINAiLimits();
  let t5;
  let t6;
  ...
  const isInOverageMode = opensinAiLimits.isUsingOverage;
  let t7;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.OPENSIN_CODE_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
