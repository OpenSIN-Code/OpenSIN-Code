...
import { FAST_MODE_MODEL_DISPLAY, isFastModeAvailable, isFastModeCooldown, isFastModeEnabled } from '../../utils_v2/fastMode.js';
import { Box, Text } from '../../ink_v2/index.js';
import { useKeybindings } from '../../keybindings_v2/useKeybinding.js';
import { useAppState, useSetAppState } from '../state/AppState.js';
import { convertEffortValueToLevel, type EffortLevel, getDefaultEffortForModel, modelSupportsEffort, modelSupportsMaxEffort, resolvePickerEffortPersistence, toPersistableEffort } from '../../utils_v2/effort.js';
import { getDefaultMainLoopModel, type ModelSetting, modelDisplayString, parseUserSpecifiedModel } from '../../utils_v2/model/model.js';
...
export type Props = {
   ...
   * project-scoped (written to the assistant's .opensin/settings.json via
   * install.ts) and should not leak to the user's global ~/.opensin/settings.
   */
...
export function ModelPicker(t0) {
  ...
  const t16 = headerText ?? "Switch between OpenSIN models. Applies to this session and future OpenSIN Code sessions. For other/previous model names, specify with --model.";
  let t17;
  if ($[42] !== t16) {
...
function EffortLevelIndicator(t0) {
  ...
  const t1 = effort ? "opensin" : "subtle";
  const t2 = effort ?? "low";
  let t3;
  if ($[0] !== t2) {
