...
async function _temp2() {
  ...
  const subscriptionType = await getExistingOpenSINSubscription();
  ...
  return {
    ...
    jsx: <Text color="suggestion">Use your existing OpenSIN {subscriptionType} plan with OpenSIN Code<Text color="text" dimColor={true}>{" "}· /login to activate</Text></Text>,
    priority: "low"
  };
}
...
async function getExistingOpenSINSubscription(): Promise<'Max' | 'Pro' | null> {
  ...
  if (profile.account.has_opensin_max) {
    return 'Max';
  }
  if (profile.account.has_opensin_pro) {
    return 'Pro';
  }
  return null;
}
