...
export function useRateLimitWarningNotification(model) {
  ...
  let t0;
  if ($[0] !== opensinAiLimits || $[1] !== model) {
    t0 = getRateLimitWarning(opensinAiLimits, model);
    $[0] = opensinAiLimits;
    $[1] = model;
    $[2] = t0;
  } else {
  ...
  let t1;
  if ($[3] !== opensinAiLimits) {
    t1 = getUsingOverageText(opensinAiLimits);
    $[3] = opensinAiLimits;
    $[4] = t1;
  } else {
    t1 = $[4];
  ...
  let t4;
  let t5;
  if ($[7] !== addNotification || $[8] !== opensinAiLimits.isUsingOverage || $[9] !== hasShownOverageNotification || $[10] !== usingOverageText) {
    t4 = () => {
      if (getIsRemoteMode()) {
        return;
      }
      if (opensinAiLimits.isUsingOverage && !hasShownOverageNotification && (!isTeamOrEnterprise || hasBillingAccess)) {
        addNotification({
          key: "limit-reached",
        ...
        if (!opensinAiLimits.isUsingOverage && hasShownOverageNotification) {
          setHasShownOverageNotification(false);
        }
      }
    };
    t5 = [opensinAiLimits.isUsingOverage, usingOverageText, hasShownOverageNotification, addNotification, hasBillingAccess, isTeamOrEnterprise];
    $[7] = addNotification;
    $[8] = opensinAiLimits.isUsingOverage;
