...
} from '../utils/inProcessTeammateHelpers'
...
import type { PaneBackendType } from '../utils/swarm/backends/types'
...
import { sendPermissionResponseViaMailbox } from '../utils/swarm/permissionSync'
import {
 ...
 * - Process-based teammates use their OPENSIN_CODE_AGENT_NAME
...
export function useInboxPoller({
  ...
  const poll = useCallback(async () => {
    ...
    // Format messages with XML wrapper for OpenSIN (include color if available)
    // Transform plan approval requests to include instructions for OpenSIN
