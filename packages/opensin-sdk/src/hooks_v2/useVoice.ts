// React hook for hold-to-talk voice input using OpenSIN voice_stream STT.
//
...
// audio module (macOS) or SoX for recording, and OpenSIN's voice_stream
// endpoint (conversation_engine) for STT.

import { useCallback, useEffect, useRef, useState } from 'react'
import { useSetVoiceState } from '../context/voice'
import { useTerminalFocus } from '../ink/hooks/use-terminal-focus'
import {
...
import { getSystemLocaleLanguage } from '../utils/intl'
...
type VoiceModule = typeof import('../services/voice')
...
export function useVoice({
  ...
  // transcripts (opensins/opensin#287008 session-sticky variant); when
      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that OpenSIN Code has microphone access.',
            )
          } else {
  ...
  useEffect(() => {
    if (enabled && !voiceModule) {
      void import('../services/voice').then(mod => {
        voiceModule = mod
      })
    }
  }, [enabled])
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    // teardown window (opensins/opensin#287008 surfaces this as
