 ...
 * For example, if the input is ['src/index', 'src/utils/helpers'],
...
/**
 * Gets additional files from OpenSIN config directories
 */
async function getOpenSINConfigFiles(cwd: string): Promise<string[]> {
  const markdownFileArrays = await Promise.all(
    OPENSIN_CONFIG_DIRECTORIES.map(subdir =>
