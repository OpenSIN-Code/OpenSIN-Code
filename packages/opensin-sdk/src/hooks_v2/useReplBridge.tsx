...
import { setMainLoopModelOverride } from '../../bootstrap_system/state.js';
...
import { getSlashCommandToolSkills, isBridgeSafeCommand } from '../../commands_v2/index.js';
...
import { buildSystemInitMessage } from '../../utils_v2/messages/systemInit.js';
import { createBridgeStatusMessage, createSystemMessage } from '../../utils_v2/messages.js';
import { getAutoModeUnavailableNotification, getAutoModeUnavailableReason, isAutoModeGateEnabled, isBypassPermissionsModeDisabled, transitionPermissionMode } from '../../utils_v2/permissions/permissionSetup.js';
 ...
 *
 * Inbound messages from opensin.ai are injected into the REPL via queuedCommands.
 */
    ...
    if (feature('BRIDGE_MODE')) {
      ...
      void (async () => {
        try {
          ...
          // Assistant mode: perpetual bridge session — opensin.ai shows one
          ...
          // When a user message arrives from opensin.ai, inject it into the REPL.
                    ...
                    void (async () => {
                      try {
                        ...
                        handleRef.current?.writeSdkMessages([buildSystemInitMessage({
                          ...
                          // OpenSIN's servers. QueryEngine (SDK) still emits
          ...
          const handle_0 = await initReplBridge({
            ...
            onSetPermissionMode(mode) {
              ...
              //     unchanged (3-way invariant violation per src/OPENSIN.md)
          ...
          if (outboundOnly) {
            ...
            if (cancelled) return;
            setMessages(prev_18 => [...prev_18, createBridgeStatusMessage(url, upgradeNudge ? 'Please upgrade to the latest version of the OpenSIN mobile app to see your Remote Control sessions.' : undefined)]);
