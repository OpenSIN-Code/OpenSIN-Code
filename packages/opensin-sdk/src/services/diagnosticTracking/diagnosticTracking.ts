...
import { OpenSINError } from '../../utils_v2/errors.js'
import { normalizePathForComparison, pathsEqual } from '../../utils_v2/file.js'
import { getConnectedIdeClient } from '../../utils_v2/ide.js'
import { jsonParse } from '../../utils_v2/slowOperations.js'

class DiagnosticsTrackingError extends OpenSINError {}
...
export class DiagnosticTrackingService {
  ...
  // Map<normalizedPath, lastOpenSINFsRightDiagnostics>
  ...
  /**
   * Get new diagnostics from file://, _opensin_fs_right, and _opensin_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    const diagnosticsForOpenSINFsRightUrisWithBaselinesMap = new Map<
      string,
      DiagnosticFile
    >()
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_opensin_fs_right:'))
      .forEach(file => {
        diagnosticsForOpenSINFsRightUrisWithBaselinesMap.set(
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _opensin_fs_right file if it exists
      const opensinFsRightFile =
        diagnosticsForOpenSINFsRightUrisWithBaselinesMap.get(normalizedPath)

      // Determine which file to use based on the state of right file diagnostics
      let fileToUse = file

      if (opensinFsRightFile) {
        const previousRightDiagnostics =
          this.rightFileDiagnosticsState.get(normalizedPath)

        // Use _opensin_fs_right if:
        ...
        if (
          ...
          !this.areDiagnosticArraysEqual(
            previousRightDiagnostics,
            opensinFsRightFile.diagnostics,
          )
        ) {
          fileToUse = opensinFsRightFile
        }
