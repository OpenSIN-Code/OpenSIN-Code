// OAuth client for handling authentication flows with OpenSIN services
...
import type { AccountInfo } from '../../utils_v2/config.js'
...
/**
 * Check if the user has OpenSIN.ai authentication scope
 * @private Only call this if you're OAuth / auth related code!
 */
export function shouldUseOpenSINAIAuth(scopes: string[] | undefined): boolean {
  return Boolean(scopes?.includes(OPENSIN_AI_INFERENCE_SCOPE))
}
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithOpenSINAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithOpenSINAi
    ? getOauthConfig().OPENSIN_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show OpenSIN Max upsell
  ...
  const scopesToUse = inferenceOnly
    ? [OPENSIN_AI_INFERENCE_SCOPE] // Long-lived inference-only tokens
    : ALL_OAUTH_SCOPES
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full OpenSIN AI set. The
  ...
  try {
    ...
    // OPENSIN_CODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
    ...
    const existing = getOpenSINAIOAuthTokens()
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.OPENSIN_CODE_ACCOUNT_UUID
  const envUserEmail = process.env.OPENSIN_CODE_USER_EMAIL
  const envOrganizationUuid = process.env.OPENSIN_CODE_ORGANIZATION_UUID
