 ...
 * - CLI commands (`opensin plugin install/uninstall/enable/disable/update`)
...
} from '../../utils_v2/plugins/cacheUtils.js'
import {
  findReverseDependents,
  formatReverseDependentsSuffix,
} from '../../utils_v2/plugins/dependencyResolver.js'
import {
...
import { deletePluginDataDir } from '../../utils_v2/plugins/pluginDirectories.js'
import {
...
} from '../../utils_v2/plugins/pluginInstallationHelpers.js'
import {
  cachePlugin,
  ...
  loadAllPlugins,
  loadPluginManifest,
} from '../../utils_v2/plugins/pluginLoader.js'
import { deletePluginOptions } from '../../utils_v2/plugins/pluginOptionsStorage.js'
...
import { calculatePluginVersion } from '../../utils_v2/plugins/pluginVersioning.js'
import type {
  PluginMarketplaceEntry,
  PluginScope,
} from '../../utils_v2/plugins/schemas.js'
import {
...
/**
 * Is this plugin enabled (value === true) in .opensin/settings.json?
 *
...
export async function uninstallPluginOp(
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      // Project scope is special: .opensin/settings.json is shared with the team.
      // Point users at the local-override escape hatch instead of --scope project.
      if (actualScope === 'project') {
        return {
          success: false,
          message: `Plugin "${plugin}" is enabled at project scope (.opensin/settings.json, shared with your team). To disable just for you: opensin plugin disable ${plugin} --scope local`,
        }
      }
      return {
        success: false,
...
export async function setPluginEnabledOp(
  ...
  // without touching the shared .opensin/settings.json).
...
async function performPluginUpdate({
  ...
  if (typeof entry.source !== 'string') {
    ...
    const manifestPath = join(sourcePath, '.opensin-plugin', 'plugin.json')
    try {
