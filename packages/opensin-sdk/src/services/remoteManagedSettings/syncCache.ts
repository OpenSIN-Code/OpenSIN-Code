...
import { OPENSIN_AI_INFERENCE_SCOPE } from '../../constants/oauth.js'
import {
 ...
 *   OPENSIN_CODE_OAUTH_TOKEN / FD, or keychain tokens missing metadata): Eligible —
...
export function isRemoteManagedSettingsEligible(): boolean {
  ...
  // Check OAuth first: most OpenSIN.ai users have no API key in the keychain.
  ...
  // Externally-injected tokens (CCD via OPENSIN_CODE_OAUTH_TOKEN, CCR via
  // OPENSIN_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR, Agent SDK, CI) carry no
  // subscriptionType metadata — getOpenSINAIOAuthTokens() constructs them with
  ...
  // Wrap in try-catch because getOpenSINApiKeyWithSource throws in CI/test environments
  // when no API key is available
  try {
    const { key: apiKey } = getOpenSINApiKeyWithSource({
