 ...
 * - OAuth users (OpenSIN.ai): Only Team and Enterprise/C4E subscribers are eligible
...
function getCachePath(): string {
  return join(getOpenSINConfigHomeDir(), CACHE_FILENAME)
}
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/opensin_code/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have OpenSIN.ai tokens
  const tokens = getOpenSINAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have OpenSIN.ai inference scope
  if (!tokens.scopes?.includes(OPENSIN_AI_INFERENCE_SCOPE)) {
    return false
  }
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for OpenSIN.ai users)
