...
import type { BetaMessageParam as MessageParam } from '@opensin-ai/sdk/resources/beta/messages/messages.mjs'
...
import { getVertexRegionForModel, isEnvTruthy } from '../../utils_v2/envUtils.js'
import { logError } from '../../utils_v2/log.js'
import { normalizeAttachmentForAPI } from '../../utils_v2/messages.js'
import {
  createBedrockRuntimeClient,
  getInferenceProfileBackingModel,
  isFoundationModel,
} from '../../utils_v2/model/bedrock.js'
import {
...
import { isToolReferenceBlock } from '../../utils_v2/toolSearch.js'
import { getAPIMetadata, getExtraBodyParams } from './api/opensin.js'
...
function stripToolSearchFieldsFromMessages(
  messages: OpenSIN.Beta.Messages.BetaMessageParam[],
): OpenSIN.Beta.Messages.BetaMessageParam[] {
  return messages.map(message => {
    ...
    const normalizedContent = message.content.map(block => {
      ...
      if (block.type === 'tool_use') {
        ...
        const toolUse =
          block as OpenSIN.Beta.Messages.BetaToolUseBlockParam & {
            caller?: unknown
          }
        return {
      ...
      if (block.type === 'tool_result') {
        const toolResult =
          block as OpenSIN.Beta.Messages.BetaToolResultBlockParam
...
export async function countTokensWithAPI(
  ...
  const message: OpenSIN.Beta.Messages.BetaMessageParam = {
    role: 'user',
    content: content,
  }
  ...
  return withTokenCountVCR(messages, tools, async () => {
    try {
      ...
      if (getAPIProvider() === 'bedrock') {
        // @opensin-sdk/bedrock-sdk doesn't support countTokens currently
      ...
      const response = await opensin.beta.messages.countTokens({
        model: normalizeModelStringForAPI(model),
        messages:
...
export async function countTokensViaHaikuFallback(
  ...
  const isBedrockWithThinking =
    isEnvTruthy(process.env.OPENSIN_CODE_USE_BEDROCK) && containsThinking
  // If we're on Vertex with thinking blocks, use Sonnet since Haiku 3.5 doesn't support thinking
  const isVertexWithThinking =
    isEnvTruthy(process.env.OPENSIN_CODE_USE_VERTEX) && containsThinking
  ...
  // Use getSmallFastModel() to respect OPENSIN_SMALL_FAST_MODEL env var for Bedrock users
  ...
  const response = await opensin.beta.messages.create({
    model: normalizeModelStringForAPI(model),
  ...
  if (
    ...
    return roughTokenCountEstimationForContent(
      message.message?.content as
        | string
        | Array<OpenSIN.ContentBlock>
        | Array<OpenSIN.ContentBlockParam>
        | undefined,
    )
  }
...
function roughTokenCountEstimationForBlock(
  block: string | OpenSIN.ContentBlock | OpenSIN.ContentBlockParam,
): number {
  ...
  if (block.type === 'image' || block.type === 'document') {
    // https://platform.opensin.com/docs/en/build-with-opensin/vision#calculate-image-costs
...
async function countTokensWithBedrock({
  ...
  try {
    ...
    const requestBody = {
      opensin_version: 'bedrock-2023-05-31',
      // When we pass tools and no messages, we need to pass a dummy message
      // to get an accurate tool token count.
      messages:
        messages.length > 0 ? messages : [{ role: 'user', content: 'foo' }],
      ...
      ...(betas.length > 0 && { opensin_beta: betas }),
      ...(containsThinking && {
        thinking: {
          type: 'enabled',
          budget_tokens: TOKEN_COUNT_THINKING_BUDGET,
