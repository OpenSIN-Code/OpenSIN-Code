...
import { withOAuth401Retry } from '../../utils_v2/http.js'
...
async function fetchBootstrapAPI(): Promise<BootstrapResponse | null> {
  ...
  const endpoint = `${getOauthConfig().BASE_API_URL}/api/opensin_cli/bootstrap`
  ...
  try {
    return await withOAuth401Retry(async () => {
      ...
      const token = getOpenSINAIOAuthTokens()?.accessToken
