import type { BetaToolUnion } from '@opensin-ai/sdk/resources/beta/messages/messages.mjs'
...
import { djb2Hash } from '../../utils_v2/hash.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getOpenSINTempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
   ...
   *  (sticky-on latched in opensin.ts). Tracked to verify the fix. */
   ...
   *  or opensin_internal.effort_override. */
  effortValue: string
  /** Hash of getExtraBodyParams() — catches OPENSIN_CODE_EXTRA_BODY and
   *  opensin_internal changes. */
...
// OpenSIN's server-side prompt cache TTL thresholds to test.
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getOpenSINTempDir(), { recursive: true })
