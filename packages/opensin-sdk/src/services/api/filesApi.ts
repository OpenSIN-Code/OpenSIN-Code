 ...
 *
 * This module provides functionality to download and upload files to OpenSIN Public Files API.
 * Used by the OpenSIN Code agent to download file attachments at session startup.
 *
 * API Reference: https://docs.opensin.com/en/api/files-content
 */
...
const OPENSIN_VERSION = '2023-06-01'

// API base URL - uses OPENSIN_BASE_URL set by env-manager for the appropriate environment
// Falls back to public API for standalone usage
function getDefaultApiBaseUrl(): string {
  return (
    process.env.OPENSIN_BASE_URL ||
    process.env.OPENSIN_CODE_API_BASE_URL ||
...
export type FilesApiConfig = {
  ...
  oauthToken: string
  /** Base URL for the API (default: https://api.opensin.com) */
  baseUrl?: string
...
/**
 * Downloads a single file from the OpenSIN Public Files API
 *
...
export async function downloadFile(
  ...
  const headers = {
    ...
    'opensin-version': OPENSIN_VERSION,
    'opensin-beta': FILES_API_BETA_HEADER,
  }
