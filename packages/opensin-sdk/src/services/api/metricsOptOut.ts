...
import { hasProfileScope, isOpenSINAISubscriber } from '../../utils_v2/auth.js'
...
import { getAuthHeaders, withOAuth401Retry } from '../../utils_v2/http.js'
import { logError } from '../../utils_v2/log.js'
import { memoizeWithTTLAsync } from '../../utils_v2/memoize.js'
...
// N `opensin -p` invocations into ~1 API call/day.
...
async function _fetchMetricsEnabled(): Promise<MetricsEnabledResponse> {
  ...
  const endpoint = `https://api.opensin.com/api/opensin_code/organizations/metrics_enabled`
...
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  ...
  if (isOpenSINAISubscriber() && !hasProfileScope()) {
    return { enabled: false, hasError: false }
  }
