...
import { getContextWindowForModel } from '../../utils_v2/context.js'
...
import { tokenCountWithEstimation } from '../../utils_v2/tokens.js'
import { getFeatureValue_CACHED_MAY_BE_STALE } from '../analytics/growthbook.js'
import { getMaxOutputTokensForModel } from '../api/opensin.js'
...
export function getEffectiveContextWindowSize(model: string): number {
  ...
  const autoCompactWindow = process.env.OPENSIN_CODE_AUTO_COMPACT_WINDOW
...
export function getAutoCompactThreshold(model: string): number {
  ...
  const envPercent = process.env.OPENSIN_AUTOCOMPACT_PCT_OVERRIDE
  if (envPercent) {
...
export function calculateTokenWarningState(
  ...
  const blockingLimitOverride = process.env.OPENSIN_CODE_BLOCKING_LIMIT_OVERRIDE
...
export async function shouldAutoCompact(
  ...
  // OPENSIN_CONTEXT_COLLAPSE env override is honored here too. require()
