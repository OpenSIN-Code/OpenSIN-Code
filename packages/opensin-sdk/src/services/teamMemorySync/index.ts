 ...
 *
 * API contract (opensin/opensin#250711 + #283027):
 *   GET  /api/opensin_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/opensin_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/opensin_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
import {
  OPENSIN_AI_INFERENCE_SCOPE,
  OPENSIN_AI_PROFILE_SCOPE,
  getOauthConfig,
  OAUTH_BETA_HEADER,
...
import { classifyAxiosError } from '../../utils_v2/errors.js'
import { getGithubRepo } from '../../utils_v2/git.js'
import {
  getAPIProvider,
...
// Per-entry size cap — server default from opensin/opensin#293258.
...
// GB-tunable per-org (opensin_code_team_memory_limits), so any compile-time
...
export type SyncState = {
   ...
   * (opensin/opensin#293258 adds error_code + extra_details.max_entries).
 ...
 * Format matches the server's entryChecksums values (opensin/opensin#283027)
...
function isUsingOAuth(): boolean {
  if (getAPIProvider() !== 'firstParty' || !isFirstPartyOpenSINBaseUrl()) {
    return false
  }
  const tokens = getOpenSINAIOAuthTokens()
  return Boolean(
    tokens?.accessToken &&
      tokens.scopes?.includes(OPENSIN_AI_INFERENCE_SCOPE) &&
      tokens.scopes.includes(OPENSIN_AI_PROFILE_SCOPE),
  )
}
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/opensin_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
 ...
 * Requires opensin/opensin#283027 deployed; on failure the caller fails the
...
async function fetchTeamMemoryHashes(
  ...
  try {
    ...
    // Requires opensin/opensin#283027. If entryChecksums is missing,
...
async function uploadTeamMemory(
  ...
  try {
    ...
    // Parse structured 413 (opensin/opensin#293258). The server's
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // 413's extra_details.max_entries — opensin/opensin#293258).  The
  // server's entry-count cap is GB-tunable per-org via
  // opensin_code_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  // Requires opensin/opensin#283027 — if the response lacks entryChecksums
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/opensinmd.js')
...
export async function pushTeamMemory(
  ...
  for (
    ...
    if (!result.conflict) {
      ...
      // max_entries (opensin/opensin#293258), cache it so the next push
    ...
    if (!probe.success || !probe.entryChecksums) {
      // Requires opensin/opensin#283027. A transient probe failure here is
