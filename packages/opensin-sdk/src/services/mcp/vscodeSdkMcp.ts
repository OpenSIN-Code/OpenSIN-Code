 ...
 * notify VSCode when files are edited or written by OpenSIN.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  const client = sdkClients.find(client => client.name === 'opensin-vscode')
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ...
      // In-band OAuth via opensin_authenticate (vs. extension-native PKCE).
