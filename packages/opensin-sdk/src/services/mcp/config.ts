...
import { getPluginMcpServers } from '../../utils_v2/plugins/mcpPluginIntegration.js'
import { loadAllPluginsCacheOnly } from '../../utils_v2/plugins/pluginLoader.js'
import { isSettingSourceEnabled } from '../../utils_v2/settings/constants.js'
import { getManagedFilePath } from '../../utils_v2/settings/managedPath.js'
import { isRestrictedToPluginOnly } from '../../utils_v2/settings/pluginOnlyPolicy.js'
import {
  getInitialSettings,
...
import {
  ...
  isMcpServerUrlEntry,
  type SettingsJson,
} from '../../utils_v2/settings/types.js'
import type { ValidationError } from '../../utils_v2/settings/validation.js'
import { jsonStringify } from '../../utils_v2/slowOperations.js'
import {
  type AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
  logEvent,
} from '../analytics/index.js'
import { fetchOpenSINAIMcpConfigsIfEligible } from './opensinai.js'
...
/**
 * CCR proxy URL path markers. In remote sessions, opensin.ai connectors arrive
 ...
 * plugin deduplication. Ignores env (plugins always inject OPENSIN_PLUGIN_ROOT)
...
/**
 * Filter opensin.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `opensin mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `opensin.ai <DisplayName>` so they never key-collide with
 ...
 * `mcp__opensin_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
 ...
 */
export function dedupOpenSINAiMcpServers(
  opensinAiServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(opensinAiServers)) {
    const sig = getMcpServerSignature(config)
    const manualDup = sig !== null ? manualSigs.get(sig) : undefined
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing opensin.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
 ...
 * in getOpenSINCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  // Block reserved server name "opensin-in-chrome"
  if (isOpenSINInChromeMCPServer(name)) {
  ...
  switch (scope) {
    ...
    case 'opensinai':
      throw new Error('Cannot add MCP server to scope: opensinai')
  }
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getOpenSINCodeMcpConfigs().
...
/**
 * Get OpenSIN Code MCP configurations (excludes opensin.ai servers from the
 ...
 * opensin.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns OpenSIN Code server configurations with appropriate scopes
 */
export async function getOpenSINCodeMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including opensin.ai servers.
 * This may be slow due to network calls - use getOpenSINCodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load opensin.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getOpenSINCodeMcpConfigs()
  }

  // Kick off the opensin.ai fetch before getOpenSINCodeMcpConfigs so it overlaps
  // with loadAllPluginsCacheOnly() inside. Memoized — the awaited call below is a cache hit.
  const opensinaiPromise = fetchOpenSINAIMcpConfigsIfEligible()
  const { servers: opensinCodeServers, errors } = await getOpenSINCodeMcpConfigs(
    {},
    opensinaiPromise,
  )
  const { allowed: opensinaiMcpServers } = filterMcpServersByPolicy(
    await opensinaiPromise,
  )

  // Suppress opensin.ai connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `opensin.ai Slack`) so the merge below
  // won't catch this — need content-based dedup by URL signature.
  const { servers: dedupedOpenSINAi } = dedupOpenSINAiMcpServers(
    opensinaiMcpServers,
    opensinCodeServers,
  )

  // Merge with opensin.ai having lowest precedence
  const servers = Object.assign({}, dedupedOpenSINAi, opensinCodeServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.opensin.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // what the best way to do this is. In the meantime, we are limiting this to opensin-vscode for now to
  // unbreak the VSCode extension for certain enterprise customers who have enterprise MCP config enabled.
  // https://opensin.slack.com/archives/C093UA0KLD7/p1764975463670109
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'opensin-vscode',
  )
}
