...
import type { AgentMcpServerInfo } from '../../components_v2/mcp/types.js'
...
import { getGlobalOpenSINFile } from '../../utils_v2/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalOpenSINFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalOpenSINFile()} [project: ${getCwd()}]`
    case 'dynamic':
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .opensin/settings.json).
  ...
  // In non-interactive mode (SDK, opensin -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: opensin.ai servers have normalized names starting with "opensin_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('opensin_ai_')) {
    return 'opensinai'
  }
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, opensinai-proxy, sse-ide, ws-ide)
