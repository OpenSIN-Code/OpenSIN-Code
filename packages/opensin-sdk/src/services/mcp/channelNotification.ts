 ...
 *   - sends `notifications/opensin/channel` notifications for inbound — this file
 *
 ...
 * Requires opensin.ai OAuth auth — API key users are blocked until
...
import { type ChannelEntry, getAllowedChannels } from '../../bootstrap_system/state.js'
...
import { escapeXmlAttr } from '../../utils_v2/xml.js'
import {
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/opensin/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['opensin/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/opensin/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/opensin/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/opensin/channel/permission_request'
 ...
 * opensin.ai auth; console orgs have no admin opt-in surface yet.
 *
 *   skip      Not a channel server, or managed org hasn't opted in, or
 *             not in session --channels. Connection stays up; handler
 *             not registered.
 *   register  Subscribe to notifications/opensin/channel.
 *
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['opensin/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/opensin/channel).
  if (!capabilities?.experimental?.['opensin/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare opensin/channel capability',
    }
  }
  ...
  if (!getOpenSINAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires opensin.ai authentication (run /login)',
    }
  }
  ...
  if (entry.kind === 'plugin') {
    // Marketplace verification: the tag is intent (plugin:slack@opensin),
    // the runtime name is just plugin:slack:X — could be slack@opensin or
