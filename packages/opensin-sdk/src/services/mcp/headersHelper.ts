...
export async function getMcpHeadersFromHelper(
  ...
  try {
    ...
    const execResult = await execFileNoThrowWithCwd(config.headersHelper, [], {
      ...
      // (git credential-helper style). See deshaw/opensin-issues#28.
      env: {
        ...process.env,
        OPENSIN_CODE_MCP_SERVER_NAME: serverName,
        OPENSIN_CODE_MCP_SERVER_URL: config.url,
      },
    })
