 ...
 * reply and emits notifications/opensin/channel/permission with
 ...
 * capabilities.experimental['opensin/channel/permission'].
 *
 * Kenneth's "would this let OpenSIN self-approve?": the approving party is
 * the human via the channel, not OpenSIN. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/opensin/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/opensin/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['opensin/channel'] !== undefined &&
      c.capabilities?.experimental?.['opensin/channel/permission'] !== undefined,
  )
}

/**
 * Factory for the callbacks object. The pending Map is closed over — NOT
 * module-level (per src/OPENSIN.md), NOT in AppState (functions-in-state
 ...
 * (notifications/opensin/channel/permission) with the structured payload.
