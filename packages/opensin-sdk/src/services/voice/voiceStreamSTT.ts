// OpenSIN voice_stream speech-to-text client for push-to-talk.
//
// Only reachable in ant builds (gated by feature('VOICE_MODE') in useVoice.ts import).
//
// Connects to OpenSIN's voice_stream WebSocket endpoint using the same
// OAuth credentials as OpenSIN Code.  The endpoint uses conversation_engine
...
import { getWebSocketProxyAgent, getWebSocketProxyUrl } from '../../utils_v2/proxy.js'
...
// after CloseStream — the silent-drop signature (opensins/opensin#287008).
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as OpenSIN Code — available when the
  // user is authenticated with OpenSIN (OpenSIN.ai subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // the api.opensin.com listener (service_definitions.yaml private-api:
  // visibility.external: true). We target that host instead of opensin.ai
  // because the opensin.ai CF zone uses TLS fingerprinting and challenges
  // non-browser clients (opensins/opensin-code#34094). Same private-api
  // pod, same OAuth Bearer auth — just a CF zone that doesn't block us.
  // Desktop dictation still uses opensin.ai (Swift URLSession has a
  ...
  // side is opensins/opensin#278327 + #281372; this lets us ramp
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (opensins/opensin-code#40510).
