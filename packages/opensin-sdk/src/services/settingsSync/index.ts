 ...
 *
 * Syncs user settings and memory files across OpenSIN Code environments.
 *
 ...
 *
 * Backend API: opensin/opensin#218817
 */
...
import { clearMemoryFileCaches } from '../../utils_v2/opensinmd.js'
...
import { markInternalWrite } from '../../utils_v2/settings/internalWrites.js'
import { getSettingsFilePathForSource } from '../../utils_v2/settings/settings.js'
import { resetSettingsCache } from '../../utils_v2/settings/settingsCache.js'
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken && tokens.scopes?.includes(OPENSIN_AI_INFERENCE_SCOPE),
  )
}

function getSettingsSyncEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/opensin_code/user_settings`
}
 ...
 * - clearMemoryFileCaches() for memory files (OPENSIN.md)
 */
