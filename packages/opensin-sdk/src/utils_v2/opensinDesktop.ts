...
export async function getOpenSINDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - OpenSIN Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/OpenSIN/opensin_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for OpenSIN Desktop config in each user directory
  ...
  throw new Error(
    'Could not find OpenSIN Desktop config file in Windows. Make sure OpenSIN Desktop is installed on Windows.',
  )
}

export async function readOpenSINDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - OpenSIN Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getOpenSINDesktopConfigPath()
