/**
 * OpenSIN Code hints protocol.
 *
 * CLIs and SDKs running under OpenSIN Code can emit a self-closing
 * `<opensin-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/opensin-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type OpenSINCodeHintType = 'plugin'

export type OpenSINCodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: OpenSINCodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<opensin-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractOpenSINCodeHints(
  output: string,
  command: string,
): { hints: OpenSINCodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<opensin-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: OpenSINCodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[opensinCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[opensinCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[opensinCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as OpenSINCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: OpenSINCodeHint | null = null
...
export function setPendingHint(hint: OpenSINCodeHint): void {
...
export function getPendingHintSnapshot(): OpenSINCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetOpenSINCodeHintStore(): void {
  pendingHint = null
