 ...
 * - Epoch management: reads worker_epoch from OPENSIN_CODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (OPENSIN_CODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * OPENSIN_CODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    OPENSIN_CODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.OPENSIN_CODE_WORKER_EPOCH
