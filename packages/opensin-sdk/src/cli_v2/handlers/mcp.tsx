 ...
 * These are dynamically imported only when the corresponding `opensin mcp *` command runs.
 */
...
export async function mcpRemoveHandler(name: string, options: {
  ...
  try {
    ...
    if (scopes.length === 0) {
      ...
      scopes.forEach(scope => {
        process.stderr.write(`  opensin mcp remove "${name}" -s ${scope}\n`);
      });
      cliError();
    }
  } catch (error) {
...
export async function mcpListHandler(): Promise<void> {
  ...
  if (Object.keys(configs).length === 0) {
    ...
    console.log('No MCP servers configured. Use `opensin mcp add` to add a server.');
  } else {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    ...
    for (const {
      ...
      } else if (server.type === 'opensinai-proxy') {
        // biome-ignore lint/suspicious/noConsole:: intentional console output
...
export async function mcpGetHandler(name: string): Promise<void> {
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.log(`\nTo remove this server, run: opensin mcp remove "${name}" -s ${server.scope}`);
  // Use gracefulShutdown to properly clean up MCP server connections
  // (process.exit bypasses cleanup handlers, leaving child processes orphaned)
  await gracefulShutdown(0);
}
...
// mcp add-from-opensin-desktop (lines 4881–4927)
export async function mcpAddFromDesktopHandler(options: {
  ...
  try {
    ...
    const {
      readOpenSINDesktopMcpServers
    } = await import('../../utils/opensinDesktop.js');
    const servers = await readOpenSINDesktopMcpServers();
    if (Object.keys(servers).length === 0) {
      cliOk('No MCP servers found in OpenSIN Desktop configuration or configuration file does not exist.');
    }
    const {
      unmount
...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  cliOk('All project-scoped (.mcp.json) server approvals and rejections have been reset.\n' + 'You will be prompted for approval next time you start OpenSIN Code.');
}
